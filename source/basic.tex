...
\grammarterm{lambda-expression}{s} appearng in a default argument of \tcode{D}
...
the default argumeht of \tcode{X::h} within the definition of \tcode{X}
...
function's class\iref{basic.scope.hiding}.
\end{note}
...
\pnum
A function declared in block scope or
a variable declared by a block scope \tcode{extern} declaration
is a member of the innermost enclosing namespace and
its name has linkage. If there is a prior declaration of
...
the block scope declaration declares that same entity and
its name receives the linkage of the previous declaration.
Otherwise, the block scope entity receives
the linkage of the innermost enclosing namespace.
\begin{example}
\begin{codeblock}
static void f();
extern "C" void h();
...
void g() {
  ...
  extern void k();              // \tcode{::k}, external linkage
...
\pnum
A block scope declaration of an entity with linkage
does not by itself make the member name
visible to any form of name lookup in its namespace scope or
eligible for declaration by \grammarterm{qualified-id}. \begin{example}
...
namespace X {
  ...
  void middle() {
    q();                        // error: \tcode{q} not visible to name lookup
  }
