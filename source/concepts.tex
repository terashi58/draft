...
whose return type is \tcode{U}. \tcode{\libconcept{CommonReference}<T, U>} is
satisfied only if:
\begin{itemize}
\item \tcode{C(t())} equals \tcode{C(t())} if and only if \tcode{t()} is an
  equality-preserving expression\iref{concepts.equality}.
...
\tcode{t} be a function whose return type is \tcode{T}, and let \tcode{u} be a
function whose return type is \tcode{U}. \tcode{\libconcept{Common}<T, U>} is
satisfied only if:
\begin{itemize}
\item \tcode{C(t())} equals \tcode{C(t())} if and only if
  \tcode{t()} is an equality-preserving
  expression\iref{concepts.equality}.
