...
the same as in \ref{algorithm.requirements}.
...
meets the \oldconcept[]{Formatter} requirements
...
as described below.
\begin{note}
Enabled specializations meet the \oldconcept[]{Formatter} requirements,
...
\oldconcept[]{Formatter} requirements.
\begin{example}
\begin{codeblock}
#include <format>
...
\pnum
A type \tcode{F} meets the \oldconceptdefn[]{Formatter} requirements if:
...
\begin{concepttable}{\oldconcept[]{Formatter} requirements}{formatter}
...
namespace std {
  ...
  private:
    ...
    template<typename T> explicit basic_format_arg(const T& v) noexcept;        // \expos
