...
an exception of type \tcode{format_error} is throw.
...
\indexlibrary{local-time-format-t@\exposid{local-time-format-it}}%
\begin{codeblock}
template<class Duration> struct @\placeholder{local-time-format-t}@ {           // \expos
...
\indexlibrary{\idxcode{local_time_format}}%
\begin{itemdecl}
template<class Duration>
  @\placeholder{local-time-format-t}@<Duration>
    local_time_format(local_time<Duration> time, const string* abbrev = nullptr,
...
\indexlibrary{\idxcode{format}!\idxcode{formatter<chrono::zoned_time>}}%
\begin{itemdecl}
template <typename FormatContext>
  typename FormatContext::iterator
    format(const chrono::zoned_time<Duration, TimeZonePtr>& tp, FormatContext& ctx);
